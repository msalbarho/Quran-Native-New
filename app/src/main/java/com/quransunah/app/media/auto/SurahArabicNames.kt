...
/**
 * Bundled unvocalized Surah short names (without "سورة ") so Auto / media
 * metadata never falls back to "سورة 1" when the mushaf DB is still warming.
 */
object SurahArabicNames {
    ...
    fun shortName(surah: Int): String? =
        NAMES.getOrNull(surah - 1)

    fun label(surah: Int): String? =
        shortName(surah)?.let { "سورة $it" }
}
